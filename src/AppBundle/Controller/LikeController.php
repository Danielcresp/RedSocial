...
class LikeController extends Controller {
	...
	public function likeAction($id = null) {
		$user = $this->getUser();/* usuario que esta identificado*/
		...
		$em->persist($like);
		$flush = $em->flush();  /*guardar start*/

		if ($flush == null) {
			/*$notification = $this->get('app.notification_service');
			$notification->set($publication->getUser(), 'like', $user->getId(), $publication->getId());*/
			
			$status = 'Te Estrella esta publicación !!';
		} else {
			$status = 'No se ha podido guardar el me Estrella !!';
		}
